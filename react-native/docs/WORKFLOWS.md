...
/release   release      →  ships the build
...
2. **Design** — `/design <screen>`. Screen spec, components list, all states. Appended to `docs/DESIGN.md`. Mockup via Artifact only if the layout is non-obvious.
3. **Build** — `/build <feature>`. Data layer (`lib/firebase/…`) → hook → component → route, in that order. Types everything. Instruments analytics. Tests happy path on iOS + Android simulator.
4. **QA** — `/test <feature>`. Spawns `qa-engineer`: RTL tests for the hook + component, edge-case sweep, bugs filed as `docs/bugs/<date>-<slug>.md`, test plan updated.
...
1. **QA sign-off** — `/test` / `/qa-sweep` runs the full pre-release checklist (`docs/TEST-PLAN.md` + qa-engineer's pre-release section). No P0/P1 outstanding.
2. **Confirm the cut** — `/scope-check` anything half-built; feature-flag it off if it's not ready.
3. **Release** — `/release <staging|production>`: runs the checklist in `docs/RELEASE.md`, calls `/bump` (version + build), tags the commit, prints the `eas build` / `eas submit` / `firebase deploy` commands.
4. **Deploy Firebase** — security rules + Cloud Functions to the target environment.
5. **Submit** — `eas submit` to TestFlight + Play internal.
6. **Smoke test the build** — install the actual store artifact on a real device (not the simulator build) and run the golden path.
7. **Promote** — if `staging` validates over 2–3 days, promote to `production`.

**Rollback:** if `production` ships a P0, hotfix off the last good tag — don't unpublish. Full procedure in `docs/PRINCIPLES.md § Release` and `docs/RELEASE.md`.
...
2. If a token changes, `/build` updates `lib/design/tokens.ts` and every component consuming it.
3. `/qa-sweep` does a visual regression pass — every screen consuming the token, light + dark.
...
- `/bump` the version in `app.json`; tag the commit.
